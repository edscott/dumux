...
ENV CFLAGS -fPIC -Wno-deprecated -mtune=generic
ENV CXXFLAGS -fPIC -Wno-deprecated -mtune=generic
ENV CXX_FLAGS -fPIC -Wno-deprecated -mtune=generic
...
RUN modules='dune-common dune-geometry dune-grid dune-localfunctions dune-istl'; \
    ...
    for module in $modules; do \
        ...
        git clone -b releases/2.5 https://gitlab.dune-project.org/core/$module.git $module; \
    done;
...
RUN modules='dune-uggrid'; \
    ...
    for module in $modules; do \
        if test -d $module; then rm -rf $module; fi; \
        git clone -b releases/2.5 https://gitlab.dune-project.org/staging/$module.git $module; \
    done;
...
RUN modules='dune-foamgrid dune-spgrid'; \
    ...
    for module in $modules; do \
        if test -d $module; then rm -rf $module; fi; \
        git clone -b releases/2.5 https://gitlab.dune-project.org/extensions/$module.git $module; \
    done;
...
# OK
RUN p='dune-common'; cd /usr/local/src/dune && dune-common/bin/dunecontrol --opts=dune-2.5.opts --only=$p configure; files=`find /usr/local/src/dune -name flags.make`; for f in $files; do  sed 's/-D-pthread//' <$f >$f.new && mv $f.new $f ; done &&  dune-common/bin/dunecontrol --only=$p  --opts=dune-2.5.opts make
# OK
RUN p='dune-geometry'; cd /usr/local/src/dune && dune-common/bin/dunecontrol  --opts=dune-2.5.opts --only=$p configure; files=`find /usr/local/src/dune -name flags.make`; for f in $files; do  sed 's/-D-pthread//' <$f >$f.new && mv $f.new $f ; done &&  dune-common/bin/dunecontrol  --opts=dune-2.5.opts --only=$p make
# OK
RUN p='dune-uggrid'; cd /usr/local/src/dune && dune-common/bin/dunecontrol  --opts=dune-2.5.opts --only=$p configure; files=`find /usr/local/src/dune -name flags.make`; for f in $files; do  sed 's/-D-pthread//' <$f >$f.new && mv $f.new $f ; done &&  dune-common/bin/dunecontrol  --opts=dune-2.5.opts --only=$p make
# OK
RUN p='dune-grid'; cd /usr/local/src/dune && dune-common/bin/dunecontrol  --opts=dune-2.5.opts --only=$p configure; files=`find /usr/local/src/dune -name flags.make`; for f in $files; do  sed 's/-D-pthread//' <$f >$f.new && mv $f.new $f ; done &&  dune-common/bin/dunecontrol  --opts=dune-2.5.opts --only=$p make
# OK
RUN p='dune-localfunctions'; cd /usr/local/src/dune && dune-common/bin/dunecontrol  --opts=dune-2.5.opts --only=$p configure; files=`find /usr/local/src/dune -name flags.make`; for f in $files; do  sed 's/-D-pthread//' <$f >$f.new && mv $f.new $f ; done &&  dune-common/bin/dunecontrol  --opts=dune-2.5.opts --only=$p make
# OK
RUN p='dune-istl'; cd /usr/local/src/dune && dune-common/bin/dunecontrol  --opts=dune-2.5.opts --only=$p configure; files=`find /usr/local/src/dune -name flags.make`; for f in $files; do  sed 's/-D-pthread//' <$f >$f.new && mv $f.new $f ; done &&  dune-common/bin/dunecontrol  --opts=dune-2.5.opts --only=$p make
# OK
RUN p='dune-spgrid'; cd /usr/local/src/dune && dune-common/bin/dunecontrol  --opts=dune-2.5.opts --only=$p configure; files=`find /usr/local/src/dune -name flags.make`; for f in $files; do  sed 's/-D-pthread//' <$f >$f.new && mv $f.new $f ; done &&  dune-common/bin/dunecontrol  --opts=dune-2.5.opts --only=$p make
# releases/2.5 does not compile with gcc7, we are using branch master from jan-11-2018
RUN p='dune-alugrid'; cd /usr/local/src/dune && dune-common/bin/dunecontrol  --opts=dune-2.5.opts --only=$p configure; files=`find /usr/local/src/dune -name flags.make`; for f in $files; do  sed 's/-D-pthread//' <$f >$f.new && mv $f.new $f ; done &&  dune-common/bin/dunecontrol  --opts=dune-2.5.opts --only=$p make
...
RUN cd /usr/local/src/dune && dune-common/bin/dunecontrol  --opts=dune-2.5.opts make install
