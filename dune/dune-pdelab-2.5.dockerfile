...
RUN modules='dune-fem'; \
    ...
    for module in $modules; do \
        ...
        git clone -b releases/2.5 https://gitlab.dune-project.org/dune-fem/$module.git $module; \
    done;
# Jed Brown workaround hack
COPY DuneFemMacros.cmake.dune-fem /usr/local/src/dune/dune-fem/cmake/modules/DuneFemMacros.cmake

RUN p='dune-fem'; cd /usr/local/src/dune && dune-common/bin/dunecontrol  --opts=dune-2.5.opts --only=$p configure; files=`find /usr/local/src/dune -name flags.make`; for f in $files; do  sed 's/-D-pthread//' <$f >$f.new && mv $f.new $f ; done &&  dune-common/bin/dunecontrol  --opts=dune-2.5.opts --only=$p make && dune-common/bin/dunecontrol  --opts=dune-2.5.opts --only=$p make install
...
RUN modules='dune-pdelab'; \
    ...
    for module in $modules; do \
        if test -d $module; then rm -rf $module; fi; \
        git clone -b releases/2.5 https://gitlab.dune-project.org/pdelab/$module.git $module; \
    done;
...
RUN modules='dune-typetree'; \
    ...
    for module in $modules; do \
        if test -d $module; then rm -rf $module; fi; \
        git clone -b releases/2.5 https://github.com/dune-project/$module.git $module; \
    done;
