...
$installationPath;
if ($ARGV[1] and $ARGV[1] =~ m/^--include=/){
    # Dumux installation directory may be specified directly:
    my ($a, $b) = split /=/, $ARGV[1], 2;
    $installationPath = $b;
    print "<!-- User specified Dumux template location:  $installationPath -->\n";
    if (not -e $installationPath){
        print "$installationPath does not exist\n";
        exit(1);
    }
} else {
    $installationPath = &getInstallationPath($ARGV[0]);
    print "<!-- Automatic Dumux template location:  $installationPath -->\n";
}
...
# FIXME: use --format argument processing for installationDir, includeDir and problemTypeTag!!!!
