...
sub printTypeTagsXML{
    ...
    if ($includePath) {$oFile =~ s/$includePath\///;}
        ...
        print OUTPUT " <typetag name=\"$typetag\" inherits=\"$inheritString\" source=\"$oFile\" realpath=\"$realpath\" $focusItem>\n";
        ...
        foreach $property (@skeys){
            ...
            if ($propertyTypeTag{$property} eq $typetag) {
                ...
                $oFile =~ s/$installationPath\///;
                if ($includePath){$oFile =~ s/$includePath\///;}
                print OUTPUT "  <property name=\"$property\" value=\"$propertyValues{$property}\" source=\"$oFile\" realpath=\"$realpath\"/>\n";
            }
        }
...
#print "typetag=$typeTag\n";
            if ($typeTagFiles{$typeTag}) {
                print("<!-- *** Warning: TypeTag \"$typeTag\"redefined at file $file --> \n");
#                exit(1);
            } else {
